...
//     ROCPPLinearDR_Ptr pLDR(new LinearDecisionRule());
//     // Approximate the adaptive decisions using the linear/constant decision rule approximator and robustify
//     ROCPPOptModelIF_Ptr RSFCModelLDR(pLDR->approximate(RSFCModel));
//
//    ROCPPRobustifyEngine_Ptr m_pRE2 (new RobustifyEngine());
//    ROCPPOptModelIF_Ptr pOut( m_pRE2->Reformulate(RSFCModelLDR) );
//
//#ifdef USE_SCIP
//    // Construct the solver; in this case, use the gurobi solver as a deterministic solver
...
//     pLDR->printOut(RSFCModelLDR, optimalSln, RSFCModel->getVar("Order_10"));
//
