...
public:
    ...
    KadaptabilityPartitionEncoderMS(boost::shared_ptr<OptimizationModelIF> pIn, string numPartitionsStr);
...
public:
    ...
    KadaptabilityApproximatorMS(boost::shared_ptr<OptimizationModelIF> pIn, uint K, double bigM = 100.0, double epsilon = 0.0001, string folder=" ");
    
    /// Constructor of K-Adaptability approximator
    /// @param pPartitionEncoder predefined partition encoder for the model to be approximated
    KadaptabilityApproximatorMS(boost::shared_ptr<KadaptabilityPartitionEncoderMS> pPartitionEncoder, double bigM = 100.0, double epsilon = 0.0001, string folder=" ");
    
    KadaptabilityApproximatorMS(boost::shared_ptr<KadaptabilityPartitionEncoderMS> pPartitionEncoder, boost::shared_ptr<BilinearTermReformulatorIF> pBTR, double epsilon = 0.0001, string folder=" ") : m_pPartitionEncoder(pPartitionEncoder), m_pBTR(pBTR), m_folder(folder), m_epsilon(epsilon) {}
    ...
    void checkCompatability(boost::shared_ptr<OptimizationModelIF> pIn);
    ...
    //boost::shared_ptr<OptimizationModelIF> fixSecondStageVariablesToWarmStart(boost::shared_ptr<OptimizationModelIF> pIn, boost::shared_ptr<OptimizationModelIF> pKadaptModel, string folderName, string slnName, uint K);
    
    //boost::shared_ptr<OptimizationModelIF> fixSecondStageVariablesToWarmStart(boost::shared_ptr<OptimizationModelIF> pIn, boost::shared_ptr<OptimizationModelIF> pKadaptModel, string folderName, string slnName, const map<uint,uint> &wsMap);
    
    //boost::shared_ptr<OptimizationModelIF> fixSecondStageVariablesToWarmStart(boost::shared_ptr<OptimizationModelIF> pIn, boost::shared_ptr<OptimizationModelIF> pKadaptModel, const map<string,double> &warmStartResults, uint K);
    ...
    boost::shared_ptr<OptimizationModelIF> fixSecondStageVariablesToWarmStart(boost::shared_ptr<OptimizationModelIF> pIn, boost::shared_ptr<OptimizationModelIF> pKadaptModel, const map<string,double> &warmStartResults, const map<uint,uint> &wsMap);
    ...
    virtual boost::shared_ptr<OptimizationModelIF> addProblemSpecificConstraints(boost::shared_ptr<OptimizationModelIF> pKadapt) const {return pKadapt;}
    ...
    boost::shared_ptr<DecisionVariableIF> getDVonPartition(string partition, string OrigDVname) const;
    ...
    boost::shared_ptr<UncertaintyIF> getUncOnPartition(string OrigUncName, string partition, uint t) const;
    
    //map<string, double> getWsSolutions(boost::shared_ptr<OptimizationModelIF> pIn, const map<string, double> &warmStartResults, uint K) const;
    ...
    void getWsSolutions(boost::shared_ptr<OptimizationModelIF> pIn, const map<string, double> &warmStartResults, const map<uint,uint> &wsMap, map<string, double> &wsSolutions);
    ...
    void printOut(boost::shared_ptr<OptimizationModelIF> pIn, const map<string, double> &resultIn, boost::shared_ptr<DecisionVariableIF> dv, string partition);
    
    /// Print out the solution for the given variable for all plans
    void printOut(boost::shared_ptr<OptimizationModelIF> pIn, const map<string, double> &resultIn, boost::shared_ptr<DecisionVariableIF> dv);
    
    /// Print out whether observe or not the given uncertainty for the given plan
    void printOut(boost::shared_ptr<OptimizationModelIF> pIn, const map<string, double> &resultIn, boost::shared_ptr<UncertaintyIF> unc, string partition);
    
    /// Print out whether we observe or not the given uncertainty for all cases
    void printOut(boost::shared_ptr<OptimizationModelIF> pIn, const map<string, double> &resultIn, boost::shared_ptr<UncertaintyIF> unc);
...
protected:
    ...
    boost::shared_ptr<MISOCP> doMyThingObjUnc(boost::shared_ptr<OptimizationModelIF> pIn);
    
    /// Method for two stage model with both objective and constraint uncertainty
    /// @warning Only two stage model is allowed here
    boost::shared_ptr<MISOCP> doMyThingCstrUnc(boost::shared_ptr<OptimizationModelIF> pIn);
    
    //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    //%%%%%%%%%%%%%%%%%%%%%% Protected Members %%%%%%%%%%%%%%%%%%%%%%
    //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    boost::shared_ptr<KadaptabilityPartitionEncoderMS> m_pPartitionEncoder;
    
    /// Map from original decision variable name to partition string to new decision variable
    map<string, map<string, boost::shared_ptr<DecisionVariableIF> > > m_DVmap; // map from original decision variable name to partition string to new decision variable
    
    /// Map from partition to map from original variable name to new variable on that partition
    map< string, map<string, boost::shared_ptr<DecisionVariableIF> > > m_mapPartitionEnc_mapOrigDVtoDVonPartition;
    ...
    map< pair<string,uint> , map<string, boost::shared_ptr<UncertaintyIF> > > m_mapPartitionEncandt_mapOrigUnctoUnconPartition;
...
public:
    ...
    LDRCDRApproximator(boost::shared_ptr<OptimizationModelIF> pIn, uint memory = 1000, uint numBits = 5, double bigM = 100., string folder=" ");
...
typedef boost::shared_ptr<ROCPPKadaptApprox> ROCPPKadaptApprox_Ptr;

typedef LDRCDRApproximator ROCPPLCDRApprox;
typedef boost::shared_ptr<ROCPPLCDRApprox> ROCPPLCDRApprox_Ptr;
